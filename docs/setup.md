...
2. Create the python envrionment:
    - Create a new conda environment with `conda create -n myenv python=3.12` (python version has to be supported by pytorch)
...
4. The Streamlit interface should now be local be available at …
    - "image working streamlit, image camera not connected streamlit"
    - when it looks like … you can proceed
    - when it looks like … the camera is not recognized and you can try to disconnect the camera and connect again or close Streamlit and restart it.

5. Calibrate the brightness and sharpness
    - An example image for good brightness and sharpness looks like this:
    - "example brighness and sharpness image"
